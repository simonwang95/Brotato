...
- `npm run verify:unlocks` 用来校验策略层的默认解锁、需解锁和掉落池文案是否与官方目录状态冲突。
...
3. 按 DLC 输入过滤非原版内容。
4. 按解锁物输入过滤非默认解锁内容。
5. 按评分器对保留下来的条目排序。评分器会同时读取候选自身的 `tags`、计划附加的 `routeTags`、手写优先级、偏好关键词、官方目录可用性、官方数值效果和模式目标。
...
DLC 角色的官方中文名来自本机深海魔怪安装包；默认/需解锁状态已用官方目录校验。精确挑战条件还未逐条补全，因此 `unlock` 文案必须继续保守标注。
