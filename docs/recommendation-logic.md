...
4. 按 DLC 输入过滤非原版内容。
5. 按解锁物输入过滤非默认解锁内容。
6. 按评分器对保留下来的条目排序。评分器会同时读取候选自身的 `tags`、计划附加的 `routeTags`、手写优先级、偏好关键词、官方目录可用性、官方数值效果和模式目标。
...
- `recommendationReasons`：最多 4 条主要命中原因，例如偏好关键词、候选标签、路线标签、手写优先级、官方目录可稳定获取或官方数值匹配角色目标。
