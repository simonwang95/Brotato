...
3. 按 DLC 输入过滤非原版内容。
4. 按解锁物输入过滤非默认解锁内容。
5. 按评分器对保留下来的条目排序。评分器会同时读取候选自身的 `tags`、计划附加的 `routeTags`、手写优先级、偏好关键词和官方目录可用性。
...
- `recommendationReasons`：最多 4 条主要命中原因，例如偏好关键词、候选标签、路线标签、手写优先级或官方目录可稳定获取。
...
DLC 角色的官方中文名来自本机深海魔怪安装包；角色解锁条件还未逐条校验，因此 `unlock` 文案必须继续保守标注。
