...
生成文件在 `data/official-unlocks.json`。该脚本只读取安装包里的静态 `challenge` / achievement 资源，不读取玩家存档，因此不受本机已解锁进度影响。当前可稳定写入 43 条原版角色精确条件；仍待校验的 2 条原版后补挑战和 9 条 DLC 挑战会保留 `challengeId`、`nameKey`、`descriptionKey`、`value`、`stat` 和 `additionalArgs`，方便继续解码 translation。`Giant / CHARACTER_GIANT` 仍保留为官方角色目录缺口。
